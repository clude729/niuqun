...
import com.daoyu.chat.db.GroupMemberDao;
import com.daoyu.chat.db.Groups;
import com.daoyu.chat.db.GroupsDao;
import com.daoyu.chat.db.UserInfoBean;
...
public class SealUserInfoManager implements OnDataListener {
    ...
    private boolean fetchFriends() throws HttpException {
        ...
        try {
            userFriendsResponse = action.getAllUserFriends();
        } catch (JSONException e) {
            NLog.d(TAG, "fetchFriends occurs JSONException e=" + e.toString());
            return true;
        }
        if (userFriendsResponse != null && HttpConstant.SUCCESS.equals(userFriendsResponse.getCode())) {
            List<UserFriendsResponse.ResultEntity> list = userFriendsResponse.getResult();
     ...
     */
    private List<Friend> addFriends(final List<UserFriendsResponse.ResultEntity> list) {
        if (list != null && list.size() > 0) {
            List<Friend> friendsList = new ArrayList<>();
            for (UserFriendsResponse.ResultEntity resultEntity : list) {
...
//                    friendsList = getFriends();
    ...
    {
        return (!TextUtils.isEmpty(userID)) && (null != getFriendByID(userID));
    }
