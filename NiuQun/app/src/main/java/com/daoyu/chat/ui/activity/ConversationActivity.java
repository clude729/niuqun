...
/**
 * 会话页面 1，设置 ActionBar title 2，加载会话页面 3，push 和 通知 判断
 */
public class ConversationActivity extends BaseActivity implements OnClickListener
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        {
            Logger.d(TAG, "onCreate, intent is empty, return!");
            return;
        }
    ...
    private void isPushMessage(Intent intent)
    {
        ...
        if (intent.getData().getScheme().equals("rong") && intent.getData().getQueryParameter("isFromPush") != null)
        {
                ...
                .equals(RongIMClient.ConnectionStatusListener.ConnectionStatus.DISCONNECTED))
            {
                ...
                {
                    Intent intent1 = new Intent(mContext, PhoneMainActivity.class);
    ...
    /**
     * 收到 push 消息后，选择进入哪个 Activity 如果程序缓存未被清理，进入 MainActivity 程序缓存被清理，进入 LoginActivity，重新获取token
     * <p>
     * 作用：由于在 manifest 中 intent-filter 是配置在 ConversationActivity 下面，所以收到消息后点击notifacition 会跳转到 DemoActivity。 以跳到
     * MainActivity 为例： 在 ConversationActivity 收到消息后，选择进入 MainActivity，这样就把 MainActivity 激活了，当你读完收到的消息点击 返回键 时，程序会退到
     * MainActivity 页面，而不是直接退回到 桌面。
     */
    private void enterActivity()
    {

        String token = SharePreferenceManager.getKeyStringValue(SharePreferenceConstant.IM_TOKEN);
    ...
    private void setAppPublicServiceActionBar(String targetId)
    {
        ...
        RongIM.getInstance().getPublicServiceProfile(Conversation.PublicServiceType.APP_PUBLIC_SERVICE, targetId,
    ...
    private void setPublicServiceActionBar(String targetId)
    {
        ...
        RongIM.getInstance().getPublicServiceProfile(Conversation.PublicServiceType.PUBLIC_SERVICE, targetId,
            new RongIMClient.ResultCallback<PublicServiceProfile>()
            {
                @Override
                public void onSuccess(PublicServiceProfile publicServiceProfile)
    ...
    {
        SealUserInfoManager.getInstance().getGroupMembers(mTargetId,
