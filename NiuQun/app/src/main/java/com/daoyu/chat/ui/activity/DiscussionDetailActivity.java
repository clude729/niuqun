    ...
    implements CompoundButton.OnCheckedChangeListener, View.OnClickListener
{
    ...
    private void initView()
    {
        ...
        RongIM.getInstance().getConversation(Conversation.ConversationType.DISCUSSION, targetId,
    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            case R.id.discu_clean:
                PromptPopupDialog.newInstance(mContext, getString(R.string.clean_discussion_chat_history))
                    ...
                    .setPromptButtonClickedListener(new PromptPopupDialog.OnPromptButtonClickedListener()
                    {
                        ...
                        public void onPositiveButtonClicked()
                        {
                            ...
                            {
                                RongIM.getInstance().clearMessages(Conversation.ConversationType.DISCUSSION, targetId,
                                ...
                                RongIMClient.getInstance().cleanRemoteHistoryMessages(
                                    Conversation.ConversationType.DISCUSSION, targetId, System.currentTimeMillis(),
                                    null);
                            }
                        }
                    }).show();
            ...
            case R.id.discu_quit:
                DialogWithYesOrNoUtils.getInstance().showDialog(mContext, "是否退出并删除当前讨论组?",
        ...
        if (resultCode == RESULT_OK)
        {
            switch (requestCode)
            {
                case SealConst.DISCUSSION_ADD_MEMBER_REQUEST_CODE:
                    ...
                    RongIMClient.getInstance().addMemberToDiscussion(targetId, addMember,
                        new RongIMClient.OperationCallback()
                        {
                            ...
                            public void onSuccess()
                            {
                                SealUserInfoManager.getInstance()
                                    .getFriends(new SealUserInfoManager.ResultCallback<List<Friend>>()
                                    {
                                        @Override
                                                ...
                                                for (Friend friend : friendList)
                                                {
                                                    for (String userId : addMember)
                                                    {
                                                        if (userId.equals(friend.getUserId()))
                                                            memberList.add(new UserInfo(userId, friend.getName(),
