...
import com.daoyu.chat.db.FriendDao;
    ...
    implements View.OnClickListener, CompoundButton.OnCheckedChangeListener
{
    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            case R.id.ac_ll_search_messages:
                Intent searchIntent = new Intent(PrivateChatDetailActivity.this,
                ...
                Friend friend = DBManager.getInstance().getDaoSession().getFriendDao().queryBuilder()
                    .where(FriendDao.Properties.UserId.eq(fromConversationId)).unique();
            ...
            case R.id.clean_friend:
                PromptPopupDialog.newInstance(mContext, getString(R.string.clean_private_chat_history))
                        ...
                        public void onPositiveButtonClicked()
                        {
                            if (RongIM.getInstance() != null && mUserInfo != null)
                            {
                                ...
                                RongIMClient.getInstance().cleanRemoteHistoryMessages(
                                    Conversation.ConversationType.PRIVATE, mUserInfo.getUserId(),
        ...
        switch (buttonView.getId())
        {
            case R.id.sw_friend_notfaction:
                if (isChecked)
                {
                    if (mUserInfo != null)
                    {
                        OperationRong.setConverstionNotif(mContext, Conversation.ConversationType.PRIVATE,
                            mUserInfo.getUserId(), true);
                    }
                }
                else
                {
            ...
            case R.id.sw_freind_top:
                if (isChecked)
                {
                    if (mUserInfo != null)
                    {
                        OperationRong.setConversationTop(mContext, Conversation.ConversationType.PRIVATE,
                            mUserInfo.getUserId(), true);
                    }
                }
                else
    ...
    private void getState(String targetId)
    {
        ...
        {//群组列表 page 进入
            ...
            {
                RongIM.getInstance().getConversation(Conversation.ConversationType.PRIVATE, targetId,
                ...
                RongIM.getInstance().getConversationNotificationStatus(Conversation.ConversationType.PRIVATE, targetId,
