...
import com.daoyu.chat.server.pinyin.PinyinComparator;
import com.daoyu.chat.server.pinyin.SideBar;
...
public class SelectFriendsActivity extends BaseActivity implements OnClickListener {
    ...
    {
        SealUserInfoManager.getInstance().getGroupMembers(groupId,
    ...
    private void initData()
    {
        if (deleDisList != null && deleDisList.size() > 0)
        {
            ...
            {
                if (deleDisList.get(i).getUserId().contains(SharePreferenceManager.getKeyCachedUserid()))
                {
                    continue;
                }
                //TODO displayName 需要处理 暂为 null
                data_list.add(new Friend(deleDisList.get(i).getUserId(), deleDisList.get(i).getName(),
                    deleDisList.get(i).getPortraitUri(), null 
                ));
            }
            /*
             * 以下3步是标准流程 1.填充数据sourceDataList 2.过滤数据,邀请新成员时需要过滤掉已经是成员的用户,但做删除操作时不需要这一步 3.设置adapter显示
             */
            fillSourceDataList();
            ...
            public void onSuccess(List<Friend> friendList)
            {
                if (mListView != null)
                {
                    if (friendList != null && friendList.size() > 0)
                    {
                        ...
                        {
                            data_list.add(new Friend(friend.getUserId(), friend.getName(), friend.getPortraitUri(),
    ...
    private void fillSourceDataListForDeleteGroupMember()
    {
        if (deleteGroupMemberList != null && deleteGroupMemberList.size() > 0)
        {
            ...
            {
                if (deleteMember.getUserId().contains(SharePreferenceManager.getKeyCachedUserid()))
                {
                    continue;
                }
                //TODO displayName 需要处理 暂为 null
                data_list.add(
                    new Friend(deleteMember.getUserId(), deleteMember.getName(), deleteMember.getPortraitUri(), null));
            }
            fillSourceDataList();
            updateAdapter();
        }
    ...
    class StartDiscussionAdapter extends BaseAdapter implements SectionIndexer
    {
        ...
        public View getView(final int position, View convertView, ViewGroup parent)
        {
            ...
            {
                viewHolder.isSelect.setOnClickListener(new OnClickListener()
                {
                    @Override
                ...
                viewHolder.isSelect.setOnClickListener(new View.OnClickListener()
                {
                    ...
                    public void onClick(View v)
                    {
                        ...
                        else
                        {
                            ...
                            LinearLayout view = (LinearLayout) View.inflate(SelectFriendsActivity.this,
                                R.layout.item_selected_friends, null);
                            SelectableRoundedImageView asyncImageView = (SelectableRoundedImageView) view
                                .findViewById(R.id.iv_selected_friends);
    ...
    private List<Friend> filledData(List<Friend> list)
    {
        ...
        {
            Friend friendModel = new Friend(list.get(i).getUserId(), list.get(i).getName(),
                list.get(i).getPortraitUri());
            //汉字转换成拼音
            String pinyin = null;
