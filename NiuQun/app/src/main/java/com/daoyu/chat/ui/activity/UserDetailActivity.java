...
public class UserDetailActivity extends BaseActivity implements OnClickListener
{
    ...
    private void initData()
    {
        ...
        {
            RongIMClient.getInstance().getUserOnlineStatus(mFriend.getUserId(),
                new IRongCallback.IGetUserOnlineStatusCallback()
                {
                    @Override
                    public void onSuccess(final ArrayList<UserOnlineStatusInfo> userOnlineStatusInfoList)
                    {
                        if (userOnlineStatusInfoList != null && userOnlineStatusInfoList.size() > 0)
                        {
                            ...
                            for (int i = 0; i < userOnlineStatusInfoList.size(); ++i)
                            {
                                ...
                                {
                                    if (userOnlineStatusInfoList.get(i).getPlatform().getValue() > userOnlineStatusInfo
                                        .getPlatform().getValue())
                                    {
                                        userOnlineStatusInfo = userOnlineStatusInfoList.get(i);
                                    }
                                }
        ...
        {
            String mySelf = SharePreferenceManager.getKeyCachedUserid();
    ...
    private void initBlackListStatusView()
    {
        if (mIsFriendsRelationship)
        {
            ...
            rightButton.setOnClickListener(new View.OnClickListener()
            {
                ...
                {
                    RongIM.getInstance().getBlacklistStatus(mFriend.getUserId(),
                        new RongIMClient.ResultCallback<RongIMClient.BlacklistStatus>()
                        {
                            @Override
                            public void onSuccess(RongIMClient.BlacklistStatus blacklistStatus)
                            {
                                SinglePopWindow morePopWindow = new SinglePopWindow(UserDetailActivity.this, mFriend,
                                    blacklistStatus);
    ...
    public void startVoice(View view)
    {
        ...
        {
            Toast.makeText(mContext, getString(io.rong.callkit.R.string.rc_voip_call_network_error), Toast.LENGTH_SHORT)
                .show();
            return;
        }
    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            case R.id.ac_bt_add_friend:
                DialogWithYesOrNoUtils.getInstance().showEditDialog(mContext, getString(R.string.add_text),
        ...
        if (result != null)
        {
            switch (requestCode)
            {
                ...
                case SYN_USER_INFO:
                    ...
                    if (HttpConstant.SUCCESS.equals(userInfoByIdResponse.getCode()) && userInfoByIdResponse.getResult() != null
                        && mFriend.getUserId().equals(userInfoByIdResponse.getResult().getUser_id()))
                    {
                        String nickName = userInfoByIdResponse.getResult().getUser_name();
                        String portraitUri = userInfoByIdResponse.getResult().getAvatar();
                        if (hasNickNameChanged(nickName) || hasPortraitUriChanged(portraitUri))
                        {
                            ...
                            UserInfo userInfo = new UserInfo(userInfoByIdResponse.getResult().getUser_id(), nickName,
