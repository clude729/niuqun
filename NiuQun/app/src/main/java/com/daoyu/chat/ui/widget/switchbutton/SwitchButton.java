package com.daoyu.chat.ui.widget.switchbutton;
...
public class SwitchButton extends CompoundButton
{
    ...
    private void setup()
    {
        ...
        if (mOnLayout != null && mOffLayout != null)
        {
            if (mThumbMargin.top + mThumbMargin.bottom > 0)
            {
                ...
                float addition = (getMeasuredHeight() - getPaddingBottom() - getPaddingTop() - mThumbSizeF.y
                    - mThumbMargin.top - mThumbMargin.bottom) / 2;
                thumbTop += addition;
            }
        }
        ...
        float textDiffWidth = Math.min(0,
            (Math.max(mThumbSizeF.x * mBackMeasureRatio, mThumbSizeF.x + mTextWidth) - mThumbRectF.width() - mTextWidth)
                / 2);
        float textDiffHeight = Math.min(0,
            (mThumbRectF.height() + mThumbMargin.top + mThumbMargin.bottom - mTextHeight) / 2);
        mBackRectF.set(backLeft + textDiffWidth, mThumbRectF.top - mThumbMargin.top + textDiffHeight,
            backLeft + mThumbMargin.left + Math.max(mThumbSizeF.x * mBackMeasureRatio, mThumbSizeF.x + mTextWidth)
        ...
        if (mOnLayout != null)
        {
            float marginOnX = mBackRectF.left + (mBackRectF.width() - mThumbRectF.width() - mOnLayout.getWidth()) / 2
        ...
        if (mOffLayout != null)
        {
            float marginOffX = mBackRectF.right - (mBackRectF.width() - mThumbRectF.width() - mOffLayout.getWidth()) / 2
                + mThumbMargin.right - mOffLayout.getWidth() - mTextMarginH * (mThumbMargin.right > 0 ? 1 : -1);
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        {
            int alpha = (int) (255
                * (getProcess() >= 0.75 ? getProcess() * 4 - 3 : (getProcess() < 0.25 ? 1 - getProcess() * 4 : 0)));
